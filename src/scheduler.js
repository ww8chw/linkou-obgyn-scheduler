...
// ---------- 每人分佈（處理不整除 + 特休）----------
// 規則：
//  - 大家以「總班數」為基準盡量平衡（差距 ≤ 1）；總班數 = (平+假+扣減)/人數。
//  - 特休者「優先用假日班少的去減」：先把多出的假日(e+1)分給無特休者，
//    特休者落在假日較少的組，再從平日扣（大特休 -2 平、小特休 -1 平）。
//  - 特休者班數本就較少，不由他人吸收（群組總額在分配時已反映特休）。
...
export function distributePerson(count, weekday, weekend, bigLeave = 0, smallLeave = 0) {
  ...
  // 無特休者優先（取得多出的假日與多出的總班數），特休者排後 → 落在假日少組。
  const normalFirst = [...Array(n).keys()].sort((a, b) => red[a] - red[b] || a - b);

  // 假日：remE 人多 1 假，優先給無特休者。
  const e = Math.floor(weekend / n);
  const remE = weekend - e * n;
  const wend = new Array(n).fill(e);
  for (let k = 0; k < remE; k++) wend[normalFirst[k]] += 1;

  // 總班數：baseTotal，remTotal 人多 1，優先給無特休者。
  const baseTotal = Math.floor((weekday + weekend + totalRed) / n);
  const remTotal = weekday + weekend + totalRed - baseTotal * n;
  const tot = new Array(n).fill(baseTotal);
  for (let k = 0; k < remTotal; k++) tot[normalFirst[k]] += 1;

  // 平日 = 總 - 特休扣減 - 假日。
  const wk = tot.map((t, i) => Math.max(0, t - red[i] - wend[i]));
